...
def get_mask_indices(size, alpha=0.5):
    if len(size) == 3:
            C, H, W = size
            indices = torch.tensor([i*H*W + j*H + k for i in range(C) for j in range(int(alpha*H), H) for k in range(W)])
    else:
        H, W = size
        indices = torch.arange(int(alpha*H), H*W)
    return indices
...
def get_datasets(dataset, train_size, test_size, normalize, generator, n_classes=None):
    if dataset == 'mnist':
        train_dataset = datasets.MNIST(train=True, size=train_size, n_classes=n_classes, normalize=normalize)
        test_dataset = datasets.MNIST(train=False, size=test_size, n_classes=n_classes, normalize=normalize)
        img_size = (1, 28, 28)
    elif dataset == 'fmnist':
        train_dataset = datasets.FashionMNIST(train=True, size=train_size, n_classes=n_classes, normalize=normalize)
        test_dataset = datasets.FashionMNIST(train=False, size=test_size, n_classes=n_classes, normalize=normalize)
        img_size = (1, 28, 28)
    else:
        train_dataset = datasets.CIFAR10(train=True, size=train_size, n_classes=n_classes, normalize=normalize)
        test_dataset = datasets.CIFAR10(train=False, size=test_size, n_classes=n_classes, normalize=normalize)
        img_size = (3, 32, 32)
